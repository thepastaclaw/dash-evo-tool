...
As a user, I want "Max" (and the amount check behind it) to use the same wallet builder used by Core-funded asset-lock sends when shielding DASH, funding a Platform address through the Simple builder-driven form, sending directly to an identity, or funding an identity from my wallet balance, so that the amount I'm offered — or type in myself — is much less likely to be rejected for exceeding what the wallet can build at that moment.

- "Max" and amount validation both use a reservation-aware, builder-derived point-in-time ceiling, reserving the relevant operation's fee, instead of estimating from the on-screen balance. This is not an absolute sendability guarantee if wallet state changes before submission.
